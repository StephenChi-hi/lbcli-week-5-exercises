# What is the address of the P2SH script wrapping the witness redeem script in this transaction
# transaction=""

transaction=""

# Decode transaction
decoded_tx=$(bitcoin-cli -regtest decoderawtransaction "$transaction")

# Extract witness script (scriptPubKey from output) - this is the witness program
witness_script=$(echo "$decoded_tx" | jq -r '.vout[0].scriptPubKey.hex')

# Hash witness script (SHA256 + RIPEMD160) to get P2SH script hash
script_hash=$(echo -n "$witness_script" | xxd -r -p | openssl dgst -sha256 -binary | openssl dgst -rmd160 -hex | awk '{print $2}')

# Build P2SH scriptPubKey (OP_HASH160 + hash + OP_EQUAL)
p2sh_script="a914${script_hash}87"

# decode script and address 
address=$(bitcoin-cli -regtest decodescript "$p2sh_script" | jq -r '.address')

echo "$address"





