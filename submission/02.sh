# What is the redeem script in this transaction
transaction=""

# decode the transaction
decoded_tx=$(bitcoin-cli -regtest decoderawtransaction "$transaction")

# extracting redeemScript with positional indexing (last element of scriptSig asm)
redeemScript=$(echo "$decoded_tx" | jq -r '.vin[0].scriptSig.asm | split(" ") | .[-1]')

# output the redeemScript
echo "$redeemScript"
            